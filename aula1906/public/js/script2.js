...
async function listarFilmes() {
    ...
    const generoMap = new Map(generos.map(genero => [genero.id, genero.description]));
    ...
    filmes.forEach(filme => {
        ...
        tr.innerHTML = `
            <td>${filme.id}</td>
            <td>${filme.name}</td>
            <td>${generoMap.get(filme.id_genres) || 'Gênero não encontrado'}</td>
            <td>
...
async function listarGeneros() {
    ...
    generos.forEach(genero => {
        ...
        option.textContent = genero.description;
